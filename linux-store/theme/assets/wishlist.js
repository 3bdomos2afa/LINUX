    ...
    const render = async () => {
      ...
      const cards = await Promise.all(list.map(async (h) => {
        ...
        try {
          ...
          return `<article class="card glass glass--hover" data-product-card data-handle="${p.handle}"><div class="card__media"><a href="${p.url}"><img class="card__img--main" src="${img}" alt="${p.title}" width="720" height="960" loading="lazy"></a><button class="card__wish is-active" type="button" data-wishlist-toggle="${p.handle}" aria-pressed="true"><svg class="icon icon--heart-fill" width="18" height="18" viewBox="0 0 24 24"><path fill="currentColor" d="M12 20s-7-4.4-7-10a4 4 0 0 1 7-2.6A4 4 0 0 1 19 10c0 5.6-7 10-7 10Z"/></svg></button>${p.has_only_default_variant || p.variants.length === 1 ? `<div class="card__quick glass glass--solid"><button class="btn btn--cream btn--sm btn--block" type="button" data-quick-add="${v.id}">+ ${L.strings.add_to_bag}</button></div>` : ''}</div><div class="card__body"><h3 class="card__title"><a href="${p.url}">${p.title}</a></h3><p class="card__meta">${p.type || ''}</p><div class="card__price"><span class="price"><span class="price__current">${L.money(p.price)}</span></span></div></div></article>`;
        } catch { return ''; }
      }));
